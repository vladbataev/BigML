...
void to_eigen_matrix(const std::vector<std::vector<float>> data, Eigen::MatrixXd& matrix) {
    int T = data.size();
    int n = data[0].size();
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < T; ++j) {
...
int main(int argc, const char* argv[]) {
    ...
    Matrix train_matrix = Matrix::Zero(train_N, train_T);
    to_eigen_matrix(train_data, train_matrix);

    if (test_data.size() > 0) {
        int test_T = test_data.size();
        int test_N = test_data[0].size();
        Matrix test_matrix = Matrix::Zero(train_N, train_T);
